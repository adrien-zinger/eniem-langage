...
fn test_statement() {
    ...
    assert!(statement(Span::new("\"\"")).is_err());
    // call
    assert!(statement(Span::new("foo();")).is_ok());
    assert!(statement(Span::new("foo()")).is_err());
}
...
fn test_function_statement() {
    ...
    assert!(function_statement(Span::new("(  a  ,  b  ){}")).is_ok());
    assert!(function_statement(Span::new("(){ let a = \"\"; }")).is_ok());
    assert!(function_statement(Span::new("){}")).is_err());
    assert!(function_statement(Span::new("({}")).is_err());
}
...
fn test_declaration() {
    ...
    assert!(declaration(Span::new("let a = \"abc\"")).is_err());
...
fn test_assignation() {
    ...
    assert!(assignation(Span::new("a = ref b")).is_err());
}
...
fn call_statement(s: Span) -> IResult<Span, Statement> {
    ...
    let (s, _) = delimited(multispace0, alpha1, multispace0).parse(s)?;
    ...
    .parse(s)?;
    let (s, _) = delimited(multispace0, tag(")"), multispace0).parse(s)?;
    let call = Call { params, block_on: block_on.is_some() };
    let res = Statement {
        pos,
...
fn expressions(s: Span) -> IResult<Span, Vec<Expression>> {
    many1(alt((statement, declaration, assignation))).parse(s)
}

#[test]
fn test_expressions() {
    assert!(expressions(Span::new("a = (){}")).is_ok());
    assert!(expressions(Span::new("let a = \"\"")).is_err());
...
fn main() {
    ...
    declaration(Span::new(&input)).unwrap();
}
